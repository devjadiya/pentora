...
export async function GET(req: NextRequest) {
  ...
  const doc = await FirestoreService.getDoc<RepositoryDoc>("Repositories", decodedrepoName) 

  if (!doc) {
    return NextResponse.json({ error: "No such repository found" }, { status: 400 });

  }
  const fileEntry = doc.fileTree.find(({ path }) => path === decodedFile);
