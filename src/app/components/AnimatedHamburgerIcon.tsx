...
import { 
  BarChart, Book, Briefcase, FileText, GitBranch, LifeBuoy, Rocket, Shield, ChevronDown 
} from 'lucide-react';
...
const Navbar = () => {
  ...
  const MobileMenu = () => (
    ...
    >
      <div className="flex flex-col space-y-4 text-xl">
                ...
                >
                  <div className="flex flex-col space-y-2 mt-2 border-l border-white/10">
                    {menuItems[key as keyof typeof menuItems].map((item, index) => (
                      <Link key={item.title + index} href={item.href} className="text-base text-gray-400 hover:text-white p-2 pl-4" onClick={() => setIsMenuOpen(false)}>
                        {item.title}
                      </Link>
                    ))}
                  </div>
        ...
        <Link href="#docs" className="text-gray-300 hover:text-white py-3" onClick={() => setIsMenuOpen(false)}>Documentation</Link>
        <Link href="#pricing" className="text-gray-300 hover:text-white py-3" onClick={() => setIsMenuOpen(false)}>Pricing</Link>
        
        <div className="flex flex-col items-center space-y-6 pt-12">
            <Link href="#login" className="text-lg text-gray-300 hover:text-white transition-colors" onClick={() => setIsMenuOpen(false)}>Login</Link>
  ...
  return (
    <>
      ...
      <nav 
        className="sticky top-10 z-50 flex items-center justify-center w-full h-16 bg-opacity-30 bg-[#04010E]/50 border-b border-t border-white/10 backdrop-blur-md"
      >
        <div className="flex items-center justify-between w-full h-full max-w-screen-xl px-4 md:px-8 mx-auto">
          ...
          <div className="hidden md:flex items-center space-x-1">
            {Object.keys(menuItems).map((key) => (
              <div key={key} className="relative h-full flex items-center" onMouseEnter={() => setActiveDropdown(key)} onMouseLeave={() => setActiveDropdown(null)}>
