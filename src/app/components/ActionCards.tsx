...
const ObjectiveCard = ({ objective, description, isActive } : {objective: string, description: string, isActive: boolean}) => (
    <div className="bg-black/20 border border-white/10 rounded-2xl p-6 h-full flex flex-col transition-all duration-300 ease-in-out">
        <div className="flex justify-between items-start gap-4">
            <div className="flex items-center gap-4">
                ...
                <h3 className="font-bold text-base md:text-lg text-white">{objective}</h3>
            </div>
            <div className="flex-shrink-0 flex items-center gap-2 text-green-400 text-sm font-semibold bg-green-500/10 px-3 py-1.5 rounded-full">
        ...
        <div className="flex items-center gap-6 my-4 pl-14">
        ...
        <p className="text-sm text-gray-400 leading-relaxed"><strong className="text-gray-200">Objective:</strong> {description}</p>
    </div>
);
...
const ActionCards = () => {
    ...
    return (
        <section className="relative bg-[#0A0514] text-white py-32 sm:py-48 flex flex-col items-center justify-center min-h-[180vh] overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-screen-2xl mx-auto px-4 sm:px-8">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.8 }} className="text-center">
                    <p className="text-purple-400 font-semibold mb-3 tracking-wider">OUR IMPACT</p>
                    <h2 className="text-4xl md:text-5xl font-bold text-white leading-tight mb-4">AI-Driven Security in Action</h2>
                    <p className="text-base md:text-lg text-gray-400 max-w-3xl mx-auto">See how our tailored solutions help global leaders in finance, tech, and critical infrastructure defend against the most advanced cyber threats.</p>
                </motion.div>

                <div className="w-full mt-20 mb-16 flex justify-center">
                        ...
                        <div className="flex overflow-x-auto" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}><style jsx>{`.no-scrollbar::-webkit-scrollbar { display: none; }`}</style>
                            <div className="flex p-1 no-scrollbar">
                                {cybersecurityData.map((item, index) => (
                                    <button key={index} onClick={() => handleTabClick(index)} className={`relative px-6 py-3 text-sm rounded-full transition-colors duration-300 outline-none flex-shrink-0 ${activeTab === index ? 'text-white' : 'text-gray-400 hover:text-white'}`}>
                ...
                <div className="relative w-full p-4 sm:p-8 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-xl shadow-2xl shadow-black/20">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                        <div className="w-full h-full flex flex-col lg:col-span-5">
                            <div className="flex justify-between items-center mb-6 px-2">
                                <div className="flex items-center gap-3">
                                    <CreatorLogoIcon />
                                    <span className="text-sm font-semibold tracking-wider text-gray-300">{currentData.creator}</span>
                                    <a href="#" className="text-sm font-semibold text-purple-400 hover:text-purple-300 flex items-center gap-1.5 transition-colors">View Site <ExternalLinkIcon /></a>
                                </div>
                                <p className="text-sm font-semibold text-gray-300 ml-auto">{activeSlide + 1} of {currentData.slides.length}</p>
                            </div>
                            ...
                            <div className="flex-grow overflow-hidden relative h-[450px]">
                             ...
                             <div className="mt-8">
                                ...
                                <AnimatePresence mode="wait">
                                    <motion.div key={activeSlide} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.4 }} className="grid grid-cols-2 gap-6 text-left">
                                        {currentData.slides[activeSlide].stats.map(stat => (
                                            <div key={stat.label} className="bg-black/20 p-4 rounded-lg">
                                                <p className="text-3xl sm:text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-white to-gray-300">{stat.value}</p>
                                                <p className="font-semibold text-white mt-1">{stat.label}</p>
